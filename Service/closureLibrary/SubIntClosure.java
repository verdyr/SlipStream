...
public class SubIntClosure implements AtomicClosure{

	//	public Parameter In1;
	//	public Parameter In2;
    ...
    public  static String Name = OntologyVocabulary.SubIntClosure;
	private static String[]  InputPortNames = {"In1", "In2"} ;
	private static String[] OutputPortNames = {"Out"} ;

	private static String[] InputPortTypes = {"int", "int"} ;
	private static String[] OutputPortTypes = {"int"} ;
	
	String DebugFileName = "/home/verdyr/workspace/TestCloud1/src/SubIntClosure.txt";

	public String getName()
	{
		return SubIntClosure.Name;
	}
	...
	public SubIntClosure()//only the ports and URI, no parameters
	{
		setRandomURI();
	...
	public void InitializeSubIntClosure(String[] values, SingleParameter[] inputParameter, SingleParameter[] outputParameter)                                                        //
	{
		for(int i = 0; i < this.InputPortNames.length;i++)
		{
			if(values[i]!= null)
			{
			Logger.printFile("\nInitializing\n values_ " + i +"=" + values[i] , DebugFileName);
			}
			else
			{
				Logger.printFile("\nInitializing\n values_ " + i +"= null" , DebugFileName);
			}
			if(inputParameter[i]!=null)
			{
				Logger.printFile("\nInitializing\n parameter_ " + i +"uri=" + inputParameter[i].getURI()
						+ "\n parameter_ " + i +"value =" + inputParameter[i].getValueForTriple()
						+ "\n parameter_ " + i +"valid =" + inputParameter[i].getValid(), DebugFileName);
				
			}
			else
			{
				Logger.printFile("\nparameter_ " + i +"=null",DebugFileName);
				
			}
			
		}
		SingleParameter p,pout;
		for(int i = 0; i < SubIntClosure.InputPortNames.length;i++)
		{   
			...
			else//I have a parameter in the constructor and I use it
			{
				 ...
				 * */
				p = inputParameter[i];
				p.setType(InputPortTypes[i]);
				if (!(values[i] == null)) //If I know the value of the parameter I  set the value and I put Valid == true
				{
					p.setValue(values[i]);
					p.setValid(true);
				}
				else  //The parameter has not a constant value to be assigned or it is ready
				{
					if(!p.getValid())
					{
					p.setValid(false);
		...
		for(int i = 0; i < SubIntClosure.OutputPortNames.length;i++)
		{
	...
	public int run()
	{
		...
		Integer result = (Integer.parseInt(InputPorts[0].getSignal().getValueForTriple())-(Integer.parseInt(InputPorts[1].getSignal().getValueForTriple())));
		this.OutputPorts[0].getSignal().setValue(result.toString());
		this.OutputPorts[0].getSignal().setType(SubIntClosure.OutputPortTypes[0]);
	...
	public Vector<Vector<String>> SerializeTriples()
	{
		...
		triples.add(xmlTools.newTriple(this.URI,   OntologyVocabulary.ClosureHasName,OntologyVocabulary.SubIntClosure, "URI", "URI"));
		triples.add(xmlTools.newTriple(this.URI,   OntologyVocabulary.ComputationHasFunctionalBehaviour,OntologyVocabulary.SubIntClosure, "URI", "URI"));
...
//		}
	...
	public SingleParameter[] getInputParameter() {
		...
		SingleParameter[] temp = new SingleParameter[SubIntClosure.InputPortNames.length];
		for (int i = 0; i < SubIntClosure.InputPortNames.length;i++)
		{
			...
			{
				System.out.println("Closure not correctly initialized: the parameter on port " + SubIntClosure.InputPortNames[i] + " = null!");
			}
		}
		return temp;
	}
	...
	public SingleParameter[] getOutputParameter() {
		SingleParameter[] temp = new SingleParameter[SubIntClosure.OutputPortNames.length];
		for (int i = 0; i < SubIntClosure.OutputPortNames.length;i++)
		{
			...
			{
				System.out.println("Closure not correctly initialized: the parameter on port " + SubIntClosure.OutputPortNames[i] + " = null!");
			}
		}
		return temp;
	}
	...
	public Integer getInputPortIndexFromName(String portName)
	{
		...
		{
			if (SubIntClosure.InputPortNames[i].equals(portName))
			{
				return i;
			}
		}
	...
	public Integer getOutputPortIndexFromName(String portName)
	{
		...
		{
			if (SubIntClosure.OutputPortNames[i].equals(portName))
			{
				return i;
			}
		}
	...
	public String printClosure()
	{
		...
		for(int i=0; i < SubIntClosure.InputPortNames.length;i++ )
		{
			temp = temp + ("port index = " + i + " + name = " + SubIntClosure.InputPortNames[i] + "URI= " + this.InputPorts[i].getURI() + "\n");
		...
		for(int i=0; i < SubIntClosure.OutputPortNames.length;i++ )
		{
			temp = temp + ("port index = " + i + " + name = " + SubIntClosure.OutputPortNames[i] + "URI= " + this.OutputPorts[i].getURI() + "\n");
