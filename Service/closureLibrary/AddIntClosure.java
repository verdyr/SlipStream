...
public class AddIntClosure implements AtomicClosure{
    ...
    public  static String Name = OntologyVocabulary.AddIntClosure;
	...
	String DebugFileName = "/home/verdyr/workspace/TestCloud1/src/AddintClosure.txt";

	public String getName()
	{
		return AddIntClosure.Name;
	}
	...
	public AddIntClosure()//only the ports and URI, no parameters
	{
		setRandomURI();
	...
	public void InitializeAddIntClosure(String[] values, SingleParameter[] inputParameter, SingleParameter[] outputParameter)                                                        //
	{
		...
		{   
			...
			else//I have a parameter in the constructor and I use it
			{
				...
				else  //The parameter has not a constant value to be assigned or is just provided by value initially
				{
					if(!p.getValid())
					{
					p.setValid(false);
	...
	public Vector<Vector<String>> SerializeTriples()
	{
		...
		triples.add(xmlTools.newTriple(this.URI,   OntologyVocabulary.ClosureHasName,OntologyVocabulary.AddIntClosure, "URI", "URI"));
		triples.add(xmlTools.newTriple(this.URI,   OntologyVocabulary.ComputationHasFunctionalBehaviour,OntologyVocabulary.AddIntClosure, "URI", "URI"));
...
//		}
	...
	public SingleParameter[] getInputParameter() {
		...
		SingleParameter[] temp = new SingleParameter[AddIntClosure.InputPortNames.length];
		for (int i = 0; i < AddIntClosure.InputPortNames.length;i++)
		{
			...
			{
				System.out.println("Closure not correctly initialized: the parameter on port " + AddIntClosure.InputPortNames[i] + " = null!");
			}
		}
		return temp;
	}
	...
	public SingleParameter[] getOutputParameter() {
		SingleParameter[] temp = new SingleParameter[AddIntClosure.OutputPortNames.length];
		for (int i = 0; i < AddIntClosure.OutputPortNames.length;i++)
		{
			...
			{
				System.out.println("Closure not correctly initialized: the parameter on port " + AddIntClosure.OutputPortNames[i] + " = null!");
			}
		}
		return temp;
	}
	...
	public Integer getOutputPortIndexFromName(String portName)
	{
		...
		{
			if (AddIntClosure.OutputPortNames[i].equals(portName))
			{
				return i;
			}
		}
	...
	public Integer getInputPortIndexFromName(String portName)
	{
		...
		{
			if (AddIntClosure.InputPortNames[i].equals(portName))
			{
				return i;
			}
		}
	...
	public String printClosure()
	{
		...
		for(int i=0; i < AddIntClosure.InputPortNames.length;i++ )
		{
			temp = temp + ("port index = " + i + " + name = " + AddIntClosure.InputPortNames[i] + "URI= " + this.InputPorts[i].getURI() + "\n");
		...
		for(int i=0; i < AddIntClosure.OutputPortNames.length;i++ )
		{
			temp = temp + ("port index = " + i + " + name = " + AddIntClosure.OutputPortNames[i] + "URI= " + this.OutputPorts[i].getURI() + "\n");
